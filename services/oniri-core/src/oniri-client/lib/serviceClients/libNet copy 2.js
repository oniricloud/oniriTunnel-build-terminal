...
function connPiper(connection, _dst, opts = {}, stats = {}) {
  ...
  if (opts.compress) {
    ...
    l2c = opts.isServer ? gzip() : gunzip()
    c2l = opts.isServer ? gunzip() : gzip()

    // Add error handlers to compression streams to prevent uncaught exceptions
    l2c.on('error', (err) => {
      if (!destroyed) destroy(err)
    })
    c2l.on('error', (err) => {
      if (!destroyed) destroy(err)
    })
  ...
  const idleTimeoutMs = opts.idleTimeout || 10000
